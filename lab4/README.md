...
Notice that the process control block now contains the field mmu_table. This is an address in memory where the root of your table will go. Each process has its own page tables.

You will be identity mapping (virtual address will be equal to the physical address) all of your addresses, but the point of the lab is to gain experience programming an MMU.
...
3. When you find the entry for VPN[1], you need to write an entry that looks like Figure 4.15. You must make sure that the V bit is set and that RWX are NOT set, otherwise, the MMU will think this is a leaf entry. The values of PPN[1] and PPN[0] will be the memory address of your second-level page table. Notice that your second level page table's memory address must be aligned by 210.
...
3. The stack size has the same restriction as the program pointer. However, the stack size for every program is exactly 8192 bytes, which may be 2 or more pages. You can find the stack in p->regs[2] and you can find the program in p->program.
...
9. You must not use any external functions or those written for you in your test program. This is because those external functions will not be properly mapped by your mmu_map function, and hence, it will always throw a page fault. If you're not getting a page fault at the start of your program, this probably means that your mmu_enable is not enabling the MMU properly.

10. To sufficiently test your program: Make sure your test() function reads, writes, and executes a wide range of code internal to the function itself. Make the code larger and see what happens. You may even call ecall() like I have in the sample code to see if you can get it to do anything.
...
**NOTE**: You will always get an IPF when the program starts. This is because my code is trying to run a test function that is impossible for you to map. This is expected behavior. If you do not get an IPF at the start of the program, this probably means that your mmu_enable() is not functioning properly. Typing p and/or t will also cause these IPF's, but they will be in different spots. The idea is to try to give you as much information as possible.

However, when you run your testing code (command 0), you should not get any page faults if your testing code works properly.
